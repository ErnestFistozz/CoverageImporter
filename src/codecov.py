...
class CodeCovCoverage(BaseCoverage):
	...
	def file_line_coverage_array(self, commit_report, filename: str) -> list:
		if commit_report:
			try:
				data = [tuple(line_coverage) for file in commit_report['files'] if file['name'].lower() == filename.lower() for line_coverage in file['line_coverage']]
				return data
			except Exception:
				return []
		return []
	...
	def commit_report(self, commitId: str):
		try:
			url = f'https://codecov.io/api/v2/gh/{self.organisation}/repos/{self.repository}/report?sha={commitId}&branch={self.branch}'
			res = requests.get(url, verify=False)
			res.raise_for_status()
